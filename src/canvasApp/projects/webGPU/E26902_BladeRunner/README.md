...
**Arranque actualizado — fase 10 R05:** GUI oculta con botón «Abrir GUI»; cabecera, FPS y LOD efectivo siempre visibles. El GPU Score elige uno de cinco perfiles al arrancar y se comprueba una sola vez al llegar a p1 con el visor desplegado. Después no se reajusta continuamente por FPS. Manual fijo y modo Auto/Escritorio/Móvil independiente. [Comprobaciones y límites](docs/phase10/R05_SCORE.md). Paneo horizontal 2 m, vertical 0,5 m y suavidad 1,4 s. Los controles conservan sus valores al ocultarse.

**R04:** Baja, Extra baja y los cinco niveles móviles priorizan resolución y desactivan bloom global/reflejo planar. Los halos pequeños permanecen. Media/Alta/UltraAlta de escritorio restauran los efectos solicitados. Comparación disponible en «Anterior · fase 10 R03». [Presupuestos y resultados](docs/phase10/R04_RESOLUCION.md).

**Capacidad GPU y revelado:** antes de cargar el GLB se ejecuta `tyrell-compute-v1` (4096 grupos × 64 hilos × 1024 iteraciones; 3 pasadas de calentamiento y hasta 12 muestras). El diagnóstico incluye `gpuCapacity.gpuScore` en iteraciones/ms, mediana, p90, variabilidad, método y fiabilidad. Los timestamps GPU son la medida preferida; sin soporte se usa tiempo de envío/completado de cola, marcado como aproximación no fiable. Una medición fallida devuelve `null`, nunca potencia cero. La calidad inicial utiliza cortes en 25, 60, 120 y 240 millones, con el límite inferior incluido. Sin score válido se usa Baja en escritorio o Extra baja en móvil. El índice mide cálculo entero sintético y no garantiza FPS; comparar únicamente la misma versión y método. El watchdog total permite continuar tras 10 s, conservando las muestras válidas terminadas. La preparación ya no consume una ventana de muestreo de 1,5 s. Si la segunda comprobación falla o tiene menor confianza, se conserva el primer score válido.

La escena renderiza con sus efectos configurados detrás de una capa `#070807`. Se exigen 60 intervalos RAF, al menos 1,5 s, diferencia de medias entre las dos mitades ≤10 %, p95/mediana ≤1,6 y máximo/mediana ≤2 (admite el coste periódico de actualizar reflejos). Tras estabilizarse se revela durante 1,5 s; tras 12 s de espera visible se revela igualmente. `startup.reason` distingue `stable` y `timeout`. Ocultar la pestaña descarta la ventana de estabilidad y pausa el tiempo; no se exige alcanzar 60 FPS. Los controles se habilitan al terminar. Con movimiento reducido se omite la duración del fundido. Los informes conservan los tiempos y estadísticas del arranque.
...
Añade `profile=1` a la URL para habilitar medición CPU de envío y timestamps GPU. Revisión técnica > Comparar efectos y coste permite guardar/exportar la referencia y medir tres pasadas de 60 s, con 5 s de calentamiento por pasada. Cambiar ajustes, cámara o visibilidad cancela la pasada pendiente. CPU/GPU conservan ventanas cortas; FPS y p95 sí abarcan la pasada completa. Sin ese parámetro, el arranque mantiene la instrumentación GPU desactivada. La fila de FPS muestra el LOD automático y, cuando corresponde, el manual. [Resultados y límites de fase 10](docs/phase10/R02_PERFORMANCE.md).
